...
// Duet 2 & 3 sizes from: https://duet3d.dozuki.com/Wiki/Mounting_and_cooling_the_board
D3Width = 140;
D3Length=134;
D3HoleOffset=4.3;
...
module dueX25(Type=0) {
	if(Type) {
		duetoverhang(2);	// Duet 0.8.5 & DuetWiFi) puts i/o ports close the the outer edge of the 2020
		spacer(4,SpacerThickness,50,40,0); // put them in one of the vent holes
	} else {
		spacer(4,SpacerThickness,0,0,0); // put them in one of the vent holes
	}
}
...
module duet(FanNotch=2) {
	...
	mount(1);
	supports(0,FanNotch);
	spacer(4,SpacerThickness,20,30,0); // put them in one of the vent holes
}
...
module Duet3(Width,Length,HoleOffset,Thickness,Cover=0,Blower=0,MSTab)
{
	...
	if(Cover) translate([-5,-20,0]) portcover(3,89,bthick+1,Width,Length,HoleOffset);
	if(Blower) translate([-5,-55,0]) blower_adapter(20,15,48,0);
	if(Cover) spacer(2,Thickness,20,30,0); // put them in one of the vent holes
	else spacer(4,Thickness,20,30,0); // put them in one of the vent holes
}
//////////////////////////////////////////////////////////////////////////
...
module spacer(Quanity=1,Thickness,X=0,Y=0,Z=0) { // spacer to move pc board off platform
	for(a=[0:Quanity-1]) {
		difference() {
...
module platform3(Width,Length,HoleOffset) { // main platform
	difference() {
		translate([-5,-7,0]) color("cyan") cubeX([Width+10,Length+mthick+7,bthick],2);
		translate([HoleOffset,HoleOffset-1,-1]) color("red") cylinder(h=bthick*2,r=screw3t/2);
		translate([Width-HoleOffset,HoleOffset-1,-1]) color("white") cylinder(h=bthick*2,r=screw3t/2);
		translate([Width-HoleOffset,Length-HoleOffset+0.5,-1]) color("blue") cylinder(h=bthick*2,r=screw3t/2);
		translate([HoleOffset,Length-HoleOffset+0.5,-1]) color("gray") cylinder(h=bthick*2,r=screw3t/2);
		vents3();
		translate([0,screw_platform-4,bthick/2]) rotate([90,0,0]) fanmountplatform();
		//if(Side_fan) {
		//	translate([-10,D3Length/2-fan_platform/2,bthick/2]) rotate([0,90,0]) fanmountside();
		//	translate([bwidth-16,D3Length/2-fan_platform/2,bthick/2]) rotate([0,90,0]) fanmountside();
		//}
	}
}
...
module vents3(over) { // vent holes in platform (over not used)
	color("red") hull() {
		translate([D3Width/3-20,25,-1]) cylinder(h=mthick*2,r=D3Width/9);
		translate([D3Width/3-20,D3Length-30,-1]) cylinder(h=mthick*2,r=D3Width/9);
	}
	color("white") hull() {
		translate([2*(D3Width/3)-20,25,-1]) cylinder(h=mthick*2,r=D3Width/9);
		translate([2*(D3Width/3)-20,D3Length-30,-1]) cylinder(h=mthick*2,r=D3Width/9);
	}
	color("blue") hull() {
		translate([3*(D3Width/3)-20,25,-1]) cylinder(h=mthick*2,r=D3Width/9);
		translate([3*(D3Width/3)-20,D3Length-30,-1]) cylinder(h=mthick*2,r=D3Width/9);
}
}
...
module mount3(Width,Length,HoleOffset,MSTab=0) { // 2020 mount
	difference() {
		translate([-5,Length,0]) color("gray") cubeX([Width+10,mthick,mheight],2);
		Holes4MS(Width,Length,HoleOffset);
	}
	if(MSTab) {
		difference() {
			rotate([-90,0,0]) translate([5,-mheight/2-3,Length+mthick]) ms_tab(Width-10);
...
module Holes4MS(Width,Length,HoleOffset) {
	translate([Width/2,Length+7,10]) rotate([90,0,0]) color("red") cylinder(h=mthick*2,r=screw5/2);
	translate([12,Length+7,10]) rotate([90,0,0]) color("white") cylinder(h=mthick*2,r=screw5/2);
	translate([Width-12,Length+7,10]) rotate([90,0,0]) color("blue") cylinder(h=mthick*2,r=screw5/2);
}
...
module supports3(Width,Length,HoleOffset) { // connects & support main platform
	difference() {
		translate([0.5,19,-mheight+sthick-3]) rotate([10,0,0])
			color("pink") cubeX([sthick,Length-10,mheight+bthick],2);
		translate([0,screw_platform+20,bthick/2]) rotate([90,0,0]) fanmountplatform();
		translate([-2,3,-sthick-19]) color("cyan") cube([mthick+5,blength+5,mheight+5]);
		translate([-5,Length+4.8,0]) color("white") cube([Width+10,10,mheight+10]);
	}
	difference() {
		translate([Width-sthick+1.5,19,-mheight+sthick-3]) rotate([10,0,0])
			color("red") cubeX([mthick,Length-10,mheight+bthick],2);
		translate([Width-sthick-2,3,-sthick-19]) color("black") cube([mthick+5,Length+5,mheight+5]);
		translate([-5,Length+4.8,0]) color("white") cube([Width+10,10,mheight+10]);
	}
	difference() {
		translate([Width/3-sthick/2,19,-mheight+sthick-3]) rotate([10,0,0])
			color("blue") cubeX([mthick,Length-10,mheight+bthick],2);
		translate([0,screw_platform+20,bthick/2]) rotate([90,0,0]) fanmountplatform();
		translate([Width/3-sthick/2-1,3,-sthick-19]) color("white") cube([mthick+5,Length+5,mheight+5]);
		translate([-5,Length+4.8,0]) color("red") cube([Width+10,10,mheight+10]);
	}
	difference() {
		translate([Width/3+Width/3-sthick/2,19,-mheight+sthick-3]) rotate([10,0,0])
			color("purple") cubeX([mthick,Length-10,mheight+bthick],2);
		translate([Width/3+Width/3-sthick/2-1,3,-sthick-19]) color("cyan") cube([mthick+5,Length+5,mheight+5]);
		translate([-5,Length+4.8,0]) color("white") cube([Width+10,10,mheight+10]);
	}
}
...
module portcover(Board=0,D3EthX,D3EthY,Width,Length,HoleOffset) { // i/o port cover for the duet 085 & Duet 2, & Duet 3
	...
	rotate([90,0,0]) {
		if(Board==0) { // Duet 2 ethernet
			...
			mounting_base(Width,Length,HoleOffset,1);
		}
...
module mounting_base(Width,Length,HoleOffset) {
	rotate([-90,0,0]) difference() {	// mounting base, it replaces the two spacers on the i/o port end
		translate([1,-3,0]) color("purple") cubeX([Width+8,11,pcsthick]);
...
module portcovermountholes(Width,Hole_Offset) {  // mounting holes to platform
	translate([Hole_Offset,0,-1]) color("red") cylinder(h=bthick*2,r=screw3o/2);
	translate([Width-Hole_Offset,0,-1]) color("black") cylinder(h=bthick*2,r=screw3o/2);
}
